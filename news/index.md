...
- Reorganized `miRPM` as a standard R package with documented public
  functions, automated tests, continuous integration, and a pkgdown
  website.
- Added the synthetic `miRPM_example` dataset for reproducible examples,
  package testing, and documentation.
- Added multi-platform R CMD check workflows for released, development,
  and previous R versions on Linux, macOS, and Windows.

### RPM normalization

- Updated
  [`normalize_rpm()`](https://sergio30po.github.io/miRPM/reference/normalize_rpm.md)
  to normalize complete miRNA count matrices using sample-specific
  library sizes.
- By default, library sizes are calculated from the complete column sums
  of the input count matrix.
...
- Redesigned
  [`filter_mirnas()`](https://sergio30po.github.io/miRPM/reference/filter_mirnas.md)
  to combine RPM abundance, raw-count, prevalence, and minimum-sample
  criteria.
- Added filtering across all samples, any group, all groups, or a
  specified minimum number of groups.
...
- Redesigned
  ...
  for nonparametric group comparisons of filtered RPM data.
- Added Wilcoxon rank-sum testing for two independent groups.
- Added Kruskal-Wallis testing and Dunn post-hoc comparisons for
...
- Added descriptive statistics, effect-size information, and structured
  result tables.
- Added optional Excel export while allowing analyses to run without
  writing files.
...
- Redesigned
  ...
  to generate both static `ggplot2` graphics and interactive `plotly`
  visualizations.
- Redesigned
  [`individual_miRNA_plot()`](https://sergio30po.github.io/miRPM/reference/individual_miRNA_plot.md)
  to generate one expression plot per selected miRNA.
- Added sample alignment, group selection, color validation, tooltip
  information, and optional file export.
- Functions now return structured objects containing plots, data,
  selected miRNAs, missing miRNAs, groups, and output paths.

### Documentation

- Added a pkgdown website with Home, Reference, and License sections.
- Expanded the README with a complete normalization, filtering,
  statistical analysis, and visualization workflow.
- Documented the orientation and alignment requirements for count
  matrices and sample metadata.
- Clarified that RPM is a transparent abundance-scaling method and is
  not a replacement for count-based differential-expression models.
